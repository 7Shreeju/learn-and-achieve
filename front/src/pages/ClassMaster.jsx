...
if( !isLoggedIn ){
  ...
  const [endate, setSelectedDate] = useState(new Date());
  ...
  const handlesubmitadd = async (e) => {
    ...
    if (!classname && !endate) {
      ...
      try {
        const response = await fetch(`http://localhost:5000/api/classmaster/add`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({ classname, endate }),
        ...
        if (response.ok == true) {
          ...
          setClassname('');
          setSelectedDate('');
          toast.success('Class Master added Successfully');
          getdata();
          setModalopen(false);
        } else {
        ...
        console.log("Classmaster Add", error);
      }
    }
  }
  ...
  const [editendate, setEditendate] = useState(null);
  ...
  //for delete
    const handleDelete =(par) => {
      setItemIdToDelete(par);    
    }

    const handleyesno =async (id) =>{
      try {
        const response = await fetch(`http://localhost:5000/api/classmaster/delete/${id}`,{
            method:"DELETE",
        });
    ...
    const handleedit = async(id) =>{
      try {
        const response = await fetch(`http://localhost:5000/api/classmaster/getbyid/${id}`,{
            method:"GET",
        });
        const data = await response.json();
        setEditclass(data.msg[0].classname);
      //setEditendate(data.msg[0].endate);
    ...
    const handlesubmitedit = async (e) => {
      ...
      if (!editclass && !editendate) {
        ...
        try {
          const response = await fetch(`http://localhost:5000/api/classmaster/update/${itemIdToDelete}`, {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
            },
            body: JSON.stringify({ classname:editclass, endate:editendate}),
          ...
          if (response.ok == true) {
            ...
            setEditendate('');
            setItemIdToDelete('')
            toast.success('Class Master updated Successfully');
            getdata();
          ...
          console.log("Classmaster edit", error);
        }
      }
    }
//forstatus
  ...
  const handlestatus1 = async (status, id ) => {
    ...
    try {
      const response1 = await fetch(`http://localhost:5000/api/classmaster/status/${id}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (response1.ok == true) {
        getdata();
        toast.success('Class Master Status updated Successfully');
        setStatus({status:"", id:""});
...
//for datatable
    ...
    const handleSelectAll = () => {
      if (selectAll) {
        ...
        const allItemIds = classmaster.map(item => item._id);
    ...
    const handleDeleteall = async () => {
      try {
        const response = await fetch(`http://localhost:5000/api/classmaster/deleteall`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
    ...
    const columns= [
      {
        name:<div className='form-check'> <input className='form-check-input' type='checkbox'  checked={selectAll} onChange={handleSelectAll} /> </div>,
        selector: (row, index) => <div className="form-check"> <input className="form-check-input" type="checkbox"checked={selectedItems.includes(row._id)} onChange={() => handleCheckboxChange(row._id)} /> </div>,
        sortable: false  
      },
      {
        name:"Sr No.",
                ...
                <div className="card">
                  <div className="head">
                    ...
                    <div className="button-search">
                      <button className="svg-button"  onClick={() => setModalopen(true)} href="#add-class-popup" data-bs-toggle="modal">
                        Add Class
                        <span>
                          <svg
                ...
                <div className="modal-content">
                  ...
                  <div className="modal-body">
                    <form onSubmit={handlesubmitadd} >
                      <h2>Add Class</h2>
                      <div className="pop-content">
                        <input type="text" name="classname" onChange={handleChange} value={classname} placeholder="Enter module name" className="Add Module" />
                        <label htmlFor="Add Module">Class<span>*</span> </label>
                        {errors.classname &&
                          <span className="error-message">
                            {errors.classname}
                          </span>
                      ...
                      <div className="pop-content">
                        ...
                        <label htmlFor="Add Module"> Class End Date<span>*</span></label>
                        {errors.endate &&
                          <span className="error-message">
                            {errors.endate}
                          </span>
                ...
                <div className="modal-content">
                  <div className="modal-body">
                    ...
                    <div className="content-body">
                      ...
                      <div className="btn1">
                        <button type="button" className="no" data-bs-dismiss="modal" aria-label="Close">No</button>
                        <button type="button" onClick={() => handlestatus1(status.status, status.id)} className="yes"> Yes </button>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="modal-content">
                  <div className="modal-body">
                    <div className="circle">
                      <div className="round">
                        ...
                        >
                          ...
                          <defs>
                            <filter id="filter0_d_1_8046" x="0.519043" y="0.171875" width="67.021" height="72.0977" filterUnits="userSpaceOnUse"color-interpolation-filters="sRGB">
                              <feFlood
                                flood-opacity="0"
                                result="BackgroundImageFix"
                              />
                    ...
                    <div className="content-body">
                      ...
                      <div className="btn1">
                        <button type="button"className="no"data-bs-dismiss="modal" aria-label="Close" >No </button>
                        <button type="button" onClick={() => handleyesno(itemIdToDelete)} className="yes"> Yes </button>
                      </div>
                    </div>
                  </div>
                </div>
